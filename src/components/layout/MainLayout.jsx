...
// Container principal da página, agora mais simples.
// O fundo e a cor do texto são herdados do GlobalStyle.
const PageContainer = styled.div`
  display: flex;
  flex-direction: column;
  ...
  width: 100%;
  min-height: 100vh;
  padding: clamp(1rem, 3vw, 4rem) clamp(1rem, 3vw, 1.5rem); // Padding responsivo
`;

// Cabeçalho que aparece nas páginas internas.
const Header = styled.header`
  width: 100%;
  max-width: 1536px; // Mantém o alinhamento com o conteúdo
  display: flex;
  justify-content: center;
  margin-bottom: clamp(2rem, 4vw, 4rem); // Usa espaçamento responsivo
  flex-shrink: 0;
`;
...
// Conteúdo principal, agora com um único estilo para todas as páginas.
const MainContent = styled(motion.main)`
  display: flex;
  flex-direction: column;
  align-items: center;
  width: 100%;
  max-width: 1536px; // Limita a largura máxima do conteúdo
  flex-grow: 1;
`;
...
const MainLayout = () => {
  ...
  // O logo só aparece em páginas que NÃO são a home.
  const showLogo = !isHomePage;
  ...
  return (
    <PageContainer>
      ...
      <MainContent
        // A key garante a animação a cada mudança de rota
        key={location.pathname}
        variants={mainVariants}
        initial="hidden"
        animate="visible"
