...
// Tema fallback para garantir que sempre haja valores
const fallbackTheme = {
  colors: {
...
const ModalContent = styled(motion.div)`
  ...
  padding: clamp(20px, 5vw, ${(props) => getTheme(props).spacing.xl});
  border-radius: 12px;
  width: 100%;
  max-width: 600px;
  max-height: 90vh;
  overflow-y: auto;
  border: 2px solid ${(props) => getTheme(props).colors.primary};
  box-shadow: 0 0 30px rgba(250, 204, 21, 0.2);
  display: flex;
  flex-direction: column;
  box-sizing: border-box;
...
const CloseButton = styled(motion.button)`
  position: absolute;
  top: clamp(5px, 1.5vw, ${(props) => getTheme(props).spacing.md});
  right: clamp(5px, 1.5vw, ${(props) => getTheme(props).spacing.md});
  background: transparent;
  border: none;
...
const ModalTitle = styled(motion.h2)`
  color: ${(props) => getTheme(props).colors.primary};
  text-align: center;
  margin-bottom: clamp(15px, 4vw, ${(props) => getTheme(props).spacing.lg});
  font-size: clamp(1.2rem, 4vw, 1.5rem);
  display: flex;
  align-items: center;
  justify-content: center;
  gap: clamp(8px, 2vw, ${(props) => getTheme(props).spacing.sm});
  margin-top: clamp(10px, 3vw, 30px);
  flex-wrap: wrap;
...
const TimesGrid = styled.div`
  display: grid;
  grid-template-columns: repeat(2, 1fr);
  gap: clamp(10px, 3vw, ${(props) => getTheme(props).spacing.lg});
  width: 100%;
  padding: 0 5px;
  box-sizing: border-box;
  ...
  @media (min-width: 769px) and (max-width: 1024px) {
    grid-template-columns: repeat(4, 1fr);
    gap: clamp(8px, 2vw, ${(props) => getTheme(props).spacing.xl});
  }
...
const TimeCard = styled(motion.div)`
  ...
  flex-direction: column;
  align-items: center;
  gap: clamp(5px, 1.5vw, ${(props) => getTheme(props).spacing.sm});
  cursor: pointer;
  padding: clamp(8px, 2vw, ${(props) => getTheme(props).spacing.sm});
  border-radius: 8px;
  border: 2px solid transparent;
  background-color: rgba(10, 25, 47, 0.33);
  transition: all 0.2s ease;
...
// --- COMPONENTE PRINCIPAL (LÓGICA FUNCIONAL ORIGINAL) ---

const SelecionarTimeModal = ({ isOpen, onClose, onSelectTime, teams }) => {
  const { playHover, playClick } = useSound();

  // Mantém a lógica funcional original: passa team E index
  const handleSelect = (team, index) => {
    playClick();
    onSelectTime(team, index);
        ...
        >
          ...
          >
            ...
            {teams && teams.length > 0 ? (
              ...
              <EmptyState>
                <IoShirtOutline size={32} style={{ opacity: 0.5 }} />
                <span>Nenhum time disponível</span>
              </EmptyState>
            )}
          </ModalContent>
