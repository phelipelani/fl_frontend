...
const GolContraModal = ({ isOpen, onClose, jogadores, nomeTime, onSelect }) => {
  return (
    <AnimatePresence>
      {isOpen && (
        <ModalOverlay initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            ...
            <ModalTitle>Quem do {nomeTime} marcou contra?</ModalTitle>
            
            {jogadores && jogadores.length > 0 ? (
              <JogadorList>
                {jogadores.map((jogador) => (
                  <JogadorButton key={jogador.id} onClick={() => { onSelect(jogador); onClose(); }}>
                    {jogador.nome}
                  </JogadorButton>
                ))}
              </JogadorList>
              ...
              <EmptyState>
                <IoWarningOutline size={32} style={{ color: '#ef4444' }} />
                <span>Nenhum jogador disponível</span>
              </EmptyState>
            )}
          </ModalContent>
