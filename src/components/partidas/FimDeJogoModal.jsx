...
const TeamScore = styled.div`
  color: ${(props) => (props.isWinner ? props.theme.colors.accentSecondary : props.theme.colors.textSecondary)};
`;
...
const VsText = styled(motion.span)`
  font-size: 1.5rem;
...
const FimDeJogoModal = ({ isOpen, onProximaPartida, placar1, placar2 }) => {
  const { playSuccess } = useSound();
  ...
  const isTime1Winner = placar1 > placar2;
  const isTime2Winner = placar2 > placar1;
  const isDraw = placar1 === placar2;
  const titleText = isDraw ? "Jogo Empatado!" : "Fim de Jogo!";
    ...
    <AnimatePresence>
      {isOpen && (
        <ModalOverlay initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          >
            <ModalTitle>
              {isDraw ? <FaHandshake /> : <FaTrophy />} {titleText}
            </ModalTitle>
            ...
            <ScoreContainer>
              <TeamScore isWinner={isTime1Winner}>
                <TeamName>Time 1</TeamName>
                <ScoreNumber>{placar1}</ScoreNumber>
                {isTime1Winner && <TrophyIcon><FaTrophy /></TrophyIcon>}
              </TeamScore>
              ...
              <TeamScore isWinner={isTime2Winner}>
                <TeamName>Time 2</TeamName>
                <ScoreNumber>{placar2}</ScoreNumber>
                {isTime2Winner && <TrophyIcon><FaTrophy /></TrophyIcon>}
              </TeamScore>
            </ScoreContainer>

            <ButtonContainer>
              <Button onClick={onProximaPartida} icon={<FaArrowRight />} iconPosition="right">
                Próxima Partida
              </Button>
            </ButtonContainer>
          </ModalContent>
