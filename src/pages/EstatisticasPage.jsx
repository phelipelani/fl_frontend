...
import GoalDistributionChart from "../components/estatisticas/GoalDistributionChart";
...
const TabelaRow = styled(motion.div)`
  ...
  &.header {
    ...
    text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.3px;
  }
  &:not(.header):hover { background: rgba(250, 204, 21, 0.08); }
...
const CellContent = styled.span`
  ...
  ${(props) => props.position && `
    ...
    background: ${ props.position <= 3 ? "linear-gradient(135deg, #facc15, #f59e0b)" : "rgba(148, 163, 184, 0.2)" };
    color: ${props.position <= 3 ? "#0a192f" : "#94a3b8"};
    font-weight: 800; font-size: 0.8rem;
  `}

  @media (orientation: landscape) and (max-height: 500px) {
    ${(props) => props.position && `width: 22px; height: 22px; font-size: 0.7rem;`}
  }
`;
...
const HorizontalBarContainer = styled.div`
  ...
  @media (orientation: landscape) and (max-height: 500px) { gap: 6px; }
`;

const HorizontalBarItem = styled(motion.div)`
  display: flex; align-items: center; gap: 10px;
  @media (orientation: landscape) and (max-height: 500px) { gap: 8px; }
`;

const PlayerNameBar = styled.div`
  min-width: 60px; font-size: 0.8rem; color: #e6f1ff; font-weight: 600; text-align: right;
  @media (orientation: landscape) and (max-height: 500px) { min-width: 50px; font-size: 0.75rem; }
`;

const BarContainer = styled.div`
  flex: 1; height: 20px; background: rgba(30, 41, 59, 0.5);
  border-radius: 10px; overflow: hidden; position: relative;
  @media (orientation: landscape) and (max-height: 500px) { height: 16px; }
`;
...
const BarValue = styled.div`
  ...
  font-size: 0.7rem; color: #0a192f; font-weight: 700;
  @media (orientation: landscape) and (max-height: 500px) { font-size: 0.65rem; right: 6px; }
`;
...
const EstatisticasPage = () => {
  ...
  const kpis = useMemo(() => {
    ...
    const assistente = [...stats].sort(
      (a, b) => b.assistencias - a.assistencias
    )[0];
    return {
      mvp: mvp.nome,
  ...
  const ligaInfo = useMemo(() => {
    ...
    const totalJogos = stats.reduce(
      (sum, p) => sum + p.vitorias + p.empates + p.derrotas,
      0
    );
    const totalGols = stats.reduce((sum, p) => sum + p.gols, 0);
    const totalAssistencias = stats.reduce(
      (sum, p) => sum + (p.assistencias || 0),
      0
    );
    const totalJogadores = stats.length;
    const tempoBola = Math.round((totalJogos * 90) / 60);
    const mediaGolsPorMinuto =
      totalJogos > 0 ? (totalGols / (totalJogos * 90)).toFixed(2) : 0;
  ...
  const chartData = useMemo(() => {
    ...
    return stats.slice(0, 10).map((player) => ({
      nome:
        player.nome.length > 8
          ? player.nome.substring(0, 8) + ".."
          : player.nome,
  ...
  return (
    <PageContainer>
      ...
      <KpiRow variants={containerVariants} initial="hidden" animate="visible">
        <KpiCard variants={itemVariants} whileHover={{ scale: 1.03 }}>
          <KpiHeader>
      ...
      <MainGrid variants={containerVariants} initial="hidden" animate="visible">
        <LeftColumn>
          <Tabela variants={itemVariants}>
            ...
            <TabelaContainer>
              ...
              {stats.map((j, i) => (
                <TabelaRow key={j.id} variants={itemVariants}>
                  <CellContent center position={i + 1}>
                    {i + 1}
                  </CellContent>
                  <PlayerName>{j.nome}</PlayerName>
                  <CellContent center bold highlight>
                    {j.total_pontos}
                  </CellContent>
          ...
          <ChartContainer variants={itemVariants}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 20 }}>
                <XAxis dataKey="nome" stroke="#94a3b8" fontSize={10} angle={-45} textAnchor="end" height={40}/>
              ...
              <HorizontalBarContainer>
                {topArtilheiros.map((player, index) => (
                  <HorizontalBarItem key={player.id}>
                    ...
                    <BarContainer>
                      <BarFill initial={{ width: 0 }} animate={{ width: `${player.percentage}%` }} transition={{ duration: 1, delay: index * 0.1 }}>
        ...
        <RightColumn>
          <InfoRow variants={containerVariants} initial="hidden" animate="visible">
            <KpiCard variants={itemVariants} whileHover={{ scale: 1.03 }}>
              <KpiHeader><KpiIcon><FaFutbol /></KpiIcon><KpiTitle>Total Gols</KpiTitle></KpiHeader>
              <KpiValue>{ligaInfo.totalGols}</KpiValue>
            </KpiCard>
            <KpiCard variants={itemVariants} whileHover={{ scale: 1.03 }}>
              <KpiHeader><KpiIcon><FaGamepad /></KpiIcon><KpiTitle>Total Jogos</KpiTitle></KpiHeader>
              <KpiValue>{ligaInfo.totalJogos}</KpiValue>
            </KpiCard>
            <KpiCard variants={itemVariants} whileHover={{ scale: 1.03 }}>
              <KpiHeader><KpiIcon><FaHandsHelping /></KpiIcon><KpiTitle>Assistências</KpiTitle></KpiHeader>
              <KpiValue>{ligaInfo.totalAssistencias}</KpiValue>
            </KpiCard>
            <KpiCard variants={itemVariants} whileHover={{ scale: 1.03 }}>
              <KpiHeader><KpiIcon><FaUsers /></KpiIcon><KpiTitle>Jogadores</KpiTitle></KpiHeader>
              <KpiValue>{ligaInfo.totalJogadores}</KpiValue>
            </KpiCard>
            <KpiCard variants={itemVariants} whileHover={{ scale: 1.03 }}>
              <KpiHeader><KpiIcon><FaClock /></KpiIcon><KpiTitle>Tempo Bola</KpiTitle></KpiHeader>
              <KpiValue>{ligaInfo.tempoBola}h</KpiValue>
            </KpiCard>
            <KpiCard variants={itemVariants} whileHover={{ scale: 1.03 }}>
              <KpiHeader><KpiIcon><FaTrophy /></KpiIcon><KpiTitle>Gols/Min</KpiTitle></KpiHeader>
          ...
          <ChartContainer variants={itemVariants}>
            ...
            <GoalDistributionChart stats={stats} />
          </ChartContainer>
        </RightColumn>
      </MainGrid>
      <BackButton />
